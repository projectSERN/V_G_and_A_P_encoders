...
class extract_face_features():
    ...
    def extract_eye_images(self, subjects):
        """
        ...
        Returns:
            eyes_left: dictionary of left eye images extracted from each subject for each frame
            eyes_right: dictionary of right eye images extracted from each subject for each frame
            error: error message if no eyes detected (default = None)
        """
        error = None
        eyes_left = {}
        eyes_right = {}
        ...
        for index in subjects:
            # Define subject in dictionary
            eyes_left[index] = []
            eyes_right[index] = []
            
            for subject in subjects[index]:
                # Go through each frame for each subject
                
                # Extract eye patches
                self.__extract_eye_image_patches(subject)
                if subject.left_eye_color is None or subject.right_eye_color is None:
                    # If no eyes detected, disregard video
                    print('ERROR: Could not find eyes in the image')
                    return None, None, "Eyes"
                else:
                    eyes_left[index].append(torch.from_numpy(subject.left_eye_color).float())
                    eyes_right[index].append(torch.from_numpy(subject.right_eye_color).float())
                    
            # Processing of eye images
            eyes_left[index] = np.array(eyes_left[index])
            eyes_right[index] = np.array(eyes_right[index])
                
            eyes_left[index] = torch.from_numpy(eyes_left[index]).float().cuda(self.device)
            eyes_right[index] = torch.from_numpy(eyes_right[index]).float().cuda(self.device)
            
            if eyes_left[index].dim() == 1 or eyes_right[index].dim() == 1:
                # Need to investigate why error occurs
                print("ERROR: Eye images are not correct shape")
                return None, None, "Dimensions"
            
            eyes_left[index] = eyes_left[index].permute(0, 3, 1, 2)
            eyes_right[index] = eyes_right[index].permute(0, 3, 1, 2)
            ...
            eyes_left[index] = transformations(eyes_left[index])
            eyes_right[index] = transformations(eyes_right[index])
        
        return eyes_left, eyes_right, error
